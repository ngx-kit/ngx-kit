...
import { evoMqBreakpoints, EvoMqParams } from './meta';

const breakpointsError =
  'Provide breakpoints for EvoMqService:\n' +
  'providers: [\n' +
  '  {\n' +
...
export class EvoMqService {
  ...
  constructor(
    ...
    private zone: NgZone,
    @Optional() @Inject(evoMqBreakpoints) private breakpoints: any,
  ) {
    if (!this.breakpoints) {
  ...
  observe(params: EvoMqParams): Observable<boolean | null> {
    if (!this.platform.isBrowser) {
  ...
  private buildQuery(params: EvoMqParams) {
