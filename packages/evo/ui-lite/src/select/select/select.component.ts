...
import { FormControl } from '@ngx-kit/evo/form';
import { Select, SelectFilter, SelectItem, SelectOptions, SelectSearchFn } from '@ngx-kit/evo/select';
import { FocusListener } from '@ngx-kit/evo/util';
...
export class SelectComponent<M> implements OnInit, OnChanges, AfterViewInit, ControlValueAccessor {
  /**
   * List of items to pick.
   */
  @Input() items: SelectItem<M>[] = [];

  /**
   * Use filter to search through the current list of `[items]`.
   */
  @Input() filter?: SelectFilter<M>;

  /**
   * Search function will be used instead of filter. The function should return list of items to pick.
   */
  @Input() searchFn?: SelectSearchFn<M>;
   ...
   */
  @Input() options: Partial<SelectOptions> = {};
  ...
  ngOnInit() {
    this.service.itemsComponent = SelectItemsComponent;
    // Handle error state
    if (this.formControl) {
