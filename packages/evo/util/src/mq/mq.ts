...
import { mqBreakpoints, MqParams } from './meta';
...
export class Mq {
  ...
  constructor(
    ...
    private zone: NgZone,
    @Optional() @Inject(mqBreakpoints) private breakpoints: any,
  ) {
    if (!this.breakpoints) {
  ...
  observe(params: MqParams): Observable<boolean | null> {
    if (!this.platform.isBrowser) {
  ...
  private buildQuery(params: MqParams) {
