...
import { FocusListener } from '../../util/src/focus-listener/focus-listener';
import { selectDefaultOptions, SelectFilter, SelectInputView, SelectItem, SelectOptions, SelectSearchFn } from './meta';
 ...
 */
@Injectable()
export class Select<M> implements OnDestroy {
  /**
   * Is control disabled.
   */
  disabled = false;
  ...
  public readonly state = new SelectState<M>();

  /**
   * @input
   */
  private _searchFn?: SelectSearchFn<M>;

  /**
   * @input
   */
  private readonly _filter = new BehaviorSubject<SelectFilter<M> | undefined>(undefined);

  /**
   * @input
   */
  private readonly _options = new BehaviorSubject<SelectOptions>(selectDefaultOptions);
  ...
  private readonly searchItems = new BehaviorSubject<SelectItem<M>[]>([]);

  private selectRef: ElementRef;

  private readonly _inputViews = new BehaviorSubject<SelectInputView<M>[] | undefined>(undefined);
   ...
   */
  set searchFn(searchFn: SelectSearchFn<M> | undefined) {
   ...
   */
  get inputViews(): SelectInputView<M>[] {
   ...
   */
  set filter(f: SelectFilter<M> | undefined) {
    this._filter.next(f);
  }
   ...
   */
  overrideOptions(options: Partial<SelectOptions>) {
    this._options.next(options ? {...selectDefaultOptions, ...options} : {...selectDefaultOptions});
  }
   ...
   */
  updateOptions(options: Partial<SelectOptions>) {
  ...
  private processSearch(request: string): Observable<SelectItem<M>[]> {
    if (this._searchFn) {
  ...
  private getItemInputView(model: M): SelectInputView<M> {
