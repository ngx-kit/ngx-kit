...
import { ChangeDetectorRef, ElementRef, Inject, Injectable, Injector, OnDestroy, Optional } from '@angular/core';
...
import { uiSelectDefaultOptions, UiSelectFilter, UiSelectInputView, UiSelectItem, UiSelectOptions, UiSelectSearchFn } from './meta';
...
export class EvoSelect<M> implements OnDestroy {
  ...
  private readonly _inputViews = new BehaviorSubject<UiSelectInputView<M>[] | undefined>(undefined);
  ...
  private handleItemsAttachment() {
    this.itemsDisplayChanges.subscribe(display => {
      if (display) {
        ...
        const portal = new ComponentPortal(EvoSelectItemsComponent, null, injector);
